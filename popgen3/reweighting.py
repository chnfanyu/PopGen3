...
class ReweightingDS(object):
    def __init__(self):
        pass

    def get_sample_restructure(self, sample, variable_names, hid_name):
        sample.reset_index(inplace=True, drop=True)
        sample["entity"] = entity
        groupby_columns = [hid_name, "entity"] + variable_names
        sample_restruct = sample.groupby(groupby_columns).size().unstack(level=list(range(1, len(groupby_columns)))).fillna(0)
    ...
    def get_row_idx(self, sample_restruct):
        ...
        for column in sample_restruct.columns.values.tolist():
            ...
            row_idx[column] = rows
            contrib[column] = np.array(sample_restruct[column].values, order="C", dtype=int)
    ...
    def get_stacked_sample_restruct(self, sample_restruct_list):
        ...
        for sample_restruct in sample_restruct_list[1:]:
            ...
            if len_left_frame_index >= len_right_frame_index:
                stacked_sample = stacked_sample.join(sample_restruct, how="outer").fillna(0)
            else:
                stacked_sample = sample_restruct.join(stacked_sample, how="outer").fillna(0)
...
class RunReweighting(object):
    ...
    def create_ds(self):
        ...
        self.region_stacked, self.region_row_idx, self.region_contrib = self._create_ds_for_resolution(region_controls_config)
    ...
    def _create_ds_for_resolution(self, control_variables_config):
        ...
        reweighting_ds_obj = ReweightingDS()
        ...
        for entity in self.entities:
            ...
            sample_restruct = reweighting_ds_obj.get_sample_restructure(sample, variable_names, hid_name)
            sample_restruct_list.append(sample_restruct)
        stacked_sample = reweighting_ds_obj.get_stacked_sample_restruct(sample_restruct_list)
    ...
    def run_reweighting(self, region_constraints, geo_constraints):
        for region_id in self.db.region_ids:
            print(f"\t{self.procedure} for Region: {region_id}")
            geo_ids = self.db.get_geo_ids_for_region(region_id)
            sample_weights = np.ones((self.region_stacked.shape[0], len(geo_ids)), dtype=float, order="C")
            for iter in range(self.outer_iterations):
                ...
                for index, geo_id in enumerate(geo_ids):
                    sample_weights[:, index] = self._adjust_sample_weights(sample_weights[:, index], geo_constraints.loc[geo_id], iters=self.inner_iterations, geo=True)
                    if iter in self.iters_to_archive:
                        self._calculate_populate_average_deviation(geo_id, iter, sample_weights[:, index], geo_constraints.loc[geo_id])
            self._populate_sample_weights(sample_weights, region_id, geo_ids)
            print(f"\tsample_weights sum: {sample_weights.sum()}")
        ...
        for i in range(iters):
            for column in reversed(constraints.index):
                if geo:
                    ...
                    weighted_sum = sample_weights.T.dot(contrib[column]).sum()
                if weighted_sum == 0:
                    print(f"Weighted sum for column {column} in iter {i} is zero so don't adjust")
                    continue
    ...
    def _entropy_adjust_sample_weights(self, sample_weights, constraints, iters=1, geo=False):
        ...
        for i in range(iters):
            for column in reversed(constraints.index):
                weights_mul_contrib = np.dot(sample_weights, ones_array) * contrib[column] if not geo else sample_weights * contrib[column]
                root = self._find_root(contrib[column], constraints[column], weights_mul_contrib)
                adjustment = root ** contrib[column]
                sample_weights[row_idx[column]] = np.multiply(sample_weights[row_idx[column]].T, adjustment[row_idx[column]]).T
        return sample_weights
    ...
    def _find_root(self, contrib, constraint, weights_mul_contrib):
        ...
        if len(root_power) == 1:
            ...
            starting_value = 0.0
            return fsolve(self._optimizing_function, starting_value, args=(root_power, root_power_weight, constraint))
    ...
    def _calculate_populate_average_deviation(self, geo_id, iter, sample_weights, constraints):
        ...
        for column in constraints.index:
            ...
            diff_sum += np.abs(weighted_sum - constraints[column]) / constraints[column]
    ...
    def _transform_column_index(self):
        multi_index = pd.MultiIndex.from_tuples(self.region_sample_weights.columns.values, names=["region_id", "geo_id"])
