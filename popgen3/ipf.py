...
class IPF(object):
    def __init__(self, seed_all, seed, idx, marginals, ipf_config, variable_names, variables_cats, variables_cats_count):
...
class RunIPF(object):
    ...
    def run_ipf(self):
        ...
        self.region_constraints, self.region_constraints_dict, self.region_iters_convergence_dict, self.region_average_diffs_dict = self._run_ipf_for_resolution(region_marginals, region_controls_config, region_ids, region_to_sample)
        ...
        self.geo_constraints, self.geo_constraints_dict, self.geo_iters_convergence_dict, self.geo_average_diffs_dict = self._run_ipf_for_resolution(geo_marginals, geo_controls_config, geo_ids, geo_to_sample)
    ...
    def _run_ipf_for_resolution(self, marginals_at_resolution, control_variables_config, geo_ids, geo_corr_to_sample):
        ...
        for entity in self.entities:
            print(f"\tIPF for Entity: {entity} complete")
            ...
            seed_geo, seed_all, row_idx = self._create_ds_for_resolution_entity(sample, entity, variable_names, variables_count, variables_cats, self.sample_geo_name)
            constraints, iters_convergence, average_diffs = self._run_ipf_all_geos(entity, seed_geo, seed_all, row_idx, marginals, variable_names, variables_count, variables_cats, variables_cats_count, geo_ids, geo_corr_to_sample)
    ...
    def _create_ds_for_resolution_entity(self, sample, entity, variable_names, variables_count, variables_cats, sample_geo_names):
        ipf_ds_geo = IPFDS(sample, entity, variable_names, variables_count, variables_cats, sample_geo_names)
        seed_geo = ipf_ds_geo.get_seed()
        ipf_ds_all = IPFDS(sample, entity, variable_names, variables_count, variables_cats)
        seed_all = ipf_ds_all.get_seed()
        row_idx = ipf_ds_all.get_row_idx(seed_all)
        return seed_geo, seed_all, row_idx

    def _run_ipf_all_geos(self, entity, seed_geo, seed_all, row_idx, marginals, variable_names, variables_count, variables_cats, variables_cats_count, geo_ids, geo_corr_to_sample):
        ...
        for geo_id in geo_ids:
            ...
            if isinstance(sample_geo_id, pd.Series):
                ...
                raise Exception("Invalid sample_geo_id")
            marginals_geo = marginals.loc[geo_id]
            ipf_obj_geo = IPF(seed_all, seed_for_geo_id, row_idx, marginals_geo, self.ipf_config, variable_names, variables_cats, variables_cats_count)
            ipf_obj_geo.run_ipf()
    ...
    def _get_frequencies_for_resolution(self, geo_ids, constraints_dict, procedure="bucket"):
        ...
        for entity in self.housing_entities:
            print(f"\tRounding frequencies for Entity: {entity} complete")
