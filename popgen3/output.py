...
class SynPopulation(object):
    ...
    def export_outputs(self):
        ...
        print(f"\tTime elapsed for generating outputs is : {time.time() - t:.4f}")
    ...
    def _export_synthetic_population(self):
        ...
        print(f"\tTime to write syn pop files is: {time.time() - t:.4f}")
    ...
    def _return_aggregate_by_geo(self, variables, entity_type, entity):
        ...
        index_names = self.pop_syn_data[entity_type].index.names
        self.pop_syn_data[entity_type].reset_index(drop=True, inplace=True)
    ...
    def _export_summary(self):
        ...
        print(f"\tSummary creation took: {time.time() - t:.4f}")
    ...
    def _stack_marginal(self, marginal_list):
        marginal_T_list = []
        for i, marginal in enumerate(marginal_list):
            transposed = marginal.T.copy()
            transposed.index.name = f"{marginal.index.name}_{i}"
            marginal_T_list.append(transposed)
        stacked_marginal = pd.concat(marginal_T_list, axis=1)
        if stacked_marginal.index.duplicated().any():
            stacked_marginal = stacked_marginal.reset_index(drop=True)
        return stacked_marginal.T

    def _report_summary(self, geo_id_rows_syn, geo_id_frequencies, geo_id_constraints):
